...
BEGIN
	...
	select GETDATE(),
		...
		INVHED_SPOTCHECK, INVHED_VOUCHER, INVHED_STARTTIME, INVHED_COMCODE, HED_GLBATCH, INVHED_LOGLOCATION
	from T_TBLINVHEADER
	where INVHED_LOCCODE=@LOC_CODE and INVHED_INVNO=@INVOICE_NO and INVHED_MODE=@invMode
...
-- save inv header
	...
	select @date=INVHED_DATETIME, @memberCode=INVHED_MEMBER, @signOnDate=INVHED_SIGNONDATE, @terminalId=INVHED_STATION, @cashier=INVHED_CASHIER, @shiftNo=INVHED_SHITNO, @tempCashier=INVHED_TEMCASHIER, @netAmount=INVHED_NETAMT
	from T_TBLINVHEADER
	where INVHED_LOCCODE=@LOC_CODE and INVHED_INVNO=@INVOICE_NO and INVHED_MODE=@invMode
	...
	select GETDATE(),
		...
		INVPAY_US_VS_FC, INVPAY_SL_VS_FC, INVPAY_SLR_CONVERT, INVPAY_SIGNOFF
	from T_TBLINVPAYMENTS
	where INVPAY_LOCCODE=@LOC_CODE and INVPAY_INVNO=@INVOICE_NO and INVPAY_MODE=@invMode
...
BEGIN
	...
	*/
	...
	INTO #hedRemarks
	FROM OPENJSON(@hedRemarks) WITH ([INVREM_REMARKS1] [varchar](100),[INVREM_REMARKS2] [varchar](100),[INVREM_REMARKS3] [varchar](100),[INVREM_REMARKS4] [varchar](100),[INVREM_REMARKS5] [varchar](100))
...
-- save inv header
	...
	VALUES
		(@SETUP_LOCATION
           ...
           , (select top(1)
				STAT_MACHINEID
			from u_tblstations
			where STAT_ID=@terminalId and STAT_LOCCODE=@LOC_CODE)--<INVHED_TABLE, varchar(8),>
           , 0--<INVHED_VAT, numeric(18,2)(18,2),>
           , 0--<INVHED_NBT, numeric(18,2)(18,2),>
           , 0--<INVHED_TRANSFER, int,>
		   )
	...
	BEGIN
		...
		SELECT @IsActiveSalesBOM = isnull(SETUP_SALES_BOM,0)
		FROM U_TBLSETUP

		IF @IsActiveSalesBOM=1
		BEGIN
			IF EXISTS(SELECT PLU_CODE
			FROM #details, M_TBLPROMASTER
			WHERE [PRO_CODE]=PLU_CODE AND PLU_PRODUCTION_POS=1)
			BEGIN
				...
				from
					...
					UNION ALL
						...
						FROM M_TBLPROBOMHED, M_TBLPROBOMDET, (SELECT PLU_CODE, [UNIT_QTY]
							FROM #details, M_TBLPROMASTER
							WHERE [PRO_CODE]=PLU_CODE AND PLU_PRODUCTION_POS=1) F_ITEM, M_TBLPROMASTER, M_TBLPROINVENTORY
				...
				SELECT @TotAmount=SUM(DET_AMOUNT)
				FROM T_TBLSPN_POS_DETAILS
				WHERE DET_RUNNO=@INVOICE_NO
			...
			FROM M_TBLPROINVENTORY, (select DET_STOCKCODE, sum(DET_UNITQTY) AS QTY
					from T_TBLSPN_POS_DETAILS
					where DET_RUNNO=@INVOICE_NO
					group by DET_STOCKCODE) STK
